...
public class ReverseArrayInGroups {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int i = 0; i < t; i++) {
			int subArrInd = Integer.parseInt(br.readLine().split(" ")[1]) - 1;
			String[] array = br.readLine().split(" ");
			for (int j = 0, k = subArrInd; j < array.length; j++, k = k - 2) {
				k = ((j + k) > (array.length - 1)) ? ((array.length - 1) - j) : k;
				sb.append(array[j + k] + " ");
				k = (k == (-subArrInd)) ? subArrInd + 2 : k;
			}
			sb.append('\n');
		}
