...
public class NumberOfPairs {
	...
	private static StringBuffer validPairsCountArray = new StringBuffer();
		...
		for (int i = 0; i < t; i++) {
			...
			for (int j = 0; j < x.length; j++) {
				for (int k = 0; k < y.length; k++) {
					if (Math.pow(x[j], y[k]) > Math.pow(y[k], x[j])) {
						validPairsCount++;
					}
				}
			}
			validPairsCountArray.append(validPairsCount + "\n");
		}
		System.out.println(validPairsCountArray);
	}
